...
public class MatchTest {
	...
	public static void Exam() {
		...
		//Q1: allMatch() 메서드를 사용하여 리스트의 모든 요소가 "나"를 포함하는지 확인.
		 ...
		 System.out.println(fruits.stream().allMatch(s -> s.contains("나")));
		//Q2: IntStream.rangeClosed(1, 10)으로 1부터 10까지 생성 후 짝수만 필터링(filter())하여 출력, 합계를 구함.
		 IntStream arr = IntStream.rangeClosed(1, 10);
		 int ans = arr.filter(s -> s % 2 == 0).sum();
		 System.out.println(ans);
		 
		 IntStream.rangeClosed(1, 10).filter(s -> s % 2 == 0).forEach(e -> System.out.printf("%3d" , e));
		 System.out.println();

		//Q3: IntStream.of(1,2,3,4,5).average()의 결과를 orElse(0.0)을 사용하여 OptionalDouble의 값을 3.0으로 출력.
		
		 double i = IntStream.of(1,2,3,4,5).average().orElse(0.0);
