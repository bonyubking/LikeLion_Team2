...
public class PersonController {
	...
	public void run() {
		//메뉴 생성=페이지 이동
		while(true) {
			System.out.println("1. 전체출력 | 2.추가 | 3.삭제 | 4.수정 | 5.찾기 | 6.파일저장 | 7.페이징 | 0.종료");
			...
			case "2": 
				      ...
				      // if(service.searchByName(p) != null) {}입력한 이름이 존재하면 차단하는 코드 
			...
			case "3": //삭제
				      ...
				      int res0=service.deletePerson(p0);
				      view.showMessage(res0>0 ? "삭제성공" : "실패" );
				      break;
			...
			case "5":
				...
				if(found !=null) {
					...
					view.showMessage("검색된 이름의 결과가 없습니다.");
				}
				break;
			...
			case "7": //페이징 처리->페이지 번호와 크기를 입력해서 해당 범위의 목록 리턴
