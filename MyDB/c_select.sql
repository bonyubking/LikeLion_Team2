...
*/
-- 사원의 이름중 S로 시작하는 사원의 이름 출력 해보자.
SELECT ename
FROM emp
WHERE ename LIKE 'S%';   -- S로 시작하는 사원

-- 사원의 이름중 T로 끝나는 사원의 이름 출력 해보자.
SELECT ename
FROM emp
...
-- 사원의 이름중 A를 포함하는 사원, 단 중간 검색만 하고 싶다
SELECT ename
FROM emp
WHERE ename LIKE '%A%'
	AND ename NOT LIKE 'A%'   -- A를 포함하는 사원
...
-- 대소문자를 구분하고 싶다면 소문자 s로 시작하는 이름 찾아보자
SELECT ename
FROM emp
...
^[ A_Za-z0-9._%]+@[ A_Za-z0-9._%]+\.[ A-Za-z ] {2,} 이메일 패턴
^[ A_Za-z0-9._%] -> 아이디 부분alter
@[ A_Za-z0-9._%] -> 도메인 이름 부분
\. 마침표 -> 문자로 인식
[ A-Za-z ] {2,}  -> 영문자 2자 이상 -> com.net 
. 임의의 문자 1개를 나타냄
.* 임의의 문자 0개 이상
...
-- Q1 REGEXP  (TH로 끝나는 사원의 이름을 출력해 보자.  )
SELECT ename FROM emp WHERE ename REGEXP 'TH$';

-- Q1 REGEXP  (L M N 이 들어간 사원의 이름을 출력해 보자.  )
SELECT ename FROM emp WHERE ename REGEXP '[LMN]';

-- Q1 REGEXP  (4글자인 사원의 이름을 출력해 보자.  )
...
-- 어떤문자 이든지 있어도 되고 없어도 되고
SELECT 'Michael!' REGEXP '.'; -- 모든 문자  0 more 

-- new\ 문자열에서 new를 그대로 찾겠다
--  .   줄바꿈 제외 문자 1 
-- \  문자
-- line 문자열 line 찾겠다.
...
REGEXP_INSTR(검색 대상 , '정규식',시작위치, 찾을 횟수 ): 위치리턴 
*/
SELECT REGEXP_INSTR('aa aaa aaaa', 'a{3}') as res;

 -- REGEXP_REPLACE(원본,'패턴','대체문자', 시작위치 1, 변경위치 0)  사용
 -- b를 X로 바꿔줘
...
-- 영문자 소문자로 매칭 [a-z]+ 를 X로  2번째 변경해줘 
 ...
 SELECT REGEXP_REPLACE('abc def Gabcd', '[a-z]+', 'X' ,1,1)  as res01;
...
INSERT INTO mytable02 (id) VALUES
	...
	INSERT INTO mytable03 (id) VALUES
    ('가'), ('나'), ('다'), ('A'), ('B'), ('C'), ('a'), ('b'), ('c');
    
    SELECT id FROM mytable03 ORDER BY id;
    SELECT id FROM mytable03 ORDER BY id DESC;
...
-- Q1 부서별 평균 급여 구하자. AVG OVER (개별행 O), 별칭은 부서 평균
...
-- Q3 부서별 급여 합계 구하자.
	SELECT DEPTNO, SUM(SAL) OVER (PARTITION BY DEPTNO) AS '부서합계',
					SUM(SAL) OVER () AS '전체합계',
