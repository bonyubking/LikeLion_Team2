...
CREATE TABLE city_as like city;
INSERT INTO city_as SELECT * FROM city;
...
INSERT INTO city_partitioned SELECT * from city; ## error
...
-- Q7)  쿼리 성능 비교해 보자.
-- Q7-1) 파티션 적용 전 (city 테이블) Population이 1,000,000 미만인 도시를 조회하는 쿼리를 
-- SQL 캐시를 사용하지 않고 실행하여 실행 시간을 측정하시오.
SELECT SQL_NO_CACHE * FROM city WHERE Population < 1000000;
EXPLAIN  SELECT  * FROM city WHERE population < 1000000;
-- -> Filter: (city.Population < 1000000)  (cost=411 rows=1349) (actual time=0.118..1.49 rows=3841 loops=1)
-- -> Table scan on city  (cost=411 rows=4046) (actual time=0.115..1.3 rows=4079 loops=1)
...
-- '-> Filter: (city_partitioned.Population < 1000000)  (cost=390 rows=1280) (actual time=0.103..2.36 rows=3841 loops=1)\n    
-- -> Table scan on city_partitioned  (cost=390 rows=3841) (actual time=0.101..2.03 rows=3841 loops=1)\n'
...
(SELECT 'p0' AS Partition_name, COUNT(*) 
AS CNT FROM city_partitioned PARTITION(p0)) 
UNION ALL
(SELECT 'p1' AS Partition_name, COUNT(*) 
AS CNT FROM city_partitioned PARTITION(p1)) 
UNION ALL
(SELECT 'p2' AS Partition_name, COUNT(*) 
AS CNT FROM city_partitioned PARTITION(p2));

-- Q14)  파티션 범위를 기준으로 각 그룹별 최대 인구를 가진 도시의 이름과 해당 파티션 이름을 함께 조회하시오. (Window 함수 활용)

SELECT Partition_Name, Name, Population
FROM (
    SELECT 
        CASE 
            WHEN Population < 1000000 THEN 'p0'
            WHEN Population < 2000000 THEN 'p1'
            ELSE 'p2'
...
SHOW CREATE TABLE city_partitioned;