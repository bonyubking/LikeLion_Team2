...
FROM table1
INNER JOIN table2
ON (table1.com_column1 = table2.com_cloumn2);
...
USE my_emp;
-- Q1. 사원의 이름과 사원이 속한 부서이름 출력
SELECT ENAME, DNAME
FROM emp, dept
WHERE EMP.DEPTNO = DEPT.DEPTNO;

-- ANSI ( INNER JOIN = JOIN )
SELECT ENAME, DNAME
...
-- Q4 주종관계 만들어서 JOIN 해보자 OUTER JOIN(LEFT,RIGHT,FULL)


/*
 주테이블의COUNT에 맞춰서 종테이블 NULL값으로 채워짐
 종테이블COUNT가 더길면 TRUE값에 의한 행의 리턴에 의해 주테이블 값이늘어남
 둘다 NULL 값 체크
...
-- Q6 각 사원의 이름과 월급, 그리고 급여 등급을 출력해보자 = EQUI JOIN, NONEQUI JOIN
...
/* -- COST = [CPU 비용, 조인 방식, 조건문 실행] 줄이는 쿼리는?
1. 인덱스 사용 (WHERE JOIN ORDER BY )
2. WHERE 절에 사용되는 속성필드 인수 조절
3. SELECT * 행수 제한 LIMIT FETCH FIRST 구문 사용
4. 정렬 그룹화
...
-- Q8 SELF JOIN -> 테이블 하나에 같은 값을 가진 컬럼을 조인하는것
-- 사원번호, 사원이름, 관리자 사원번호, 관리자 이름 출력
