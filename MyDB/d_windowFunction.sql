...
LAST_VALUE(): 	그룹 내 마지막 값 -> 프레임(ROWS) 지정 필수적으로 해줘야한다
NTH_VALUE()	: N번째 값 가져오기
CUME_DIST()	: 누적 비율 계산 (누적 / 전체 행 ) -> 상위 %를 보고싶다
PERCENT_RANK()	: 상대적 백분율 순위 계산
 ...
 EX 3) 프레임 시작  :  2 PRECEDING  ,     프레임 끝 :  1 FOLLOWING
  . SUM(SAL) OVER ( ORDER BY HIREDATE  
    ...
    --LAG(expr [, N[, default]]) [null_treament] over_clause
	--LAG(칼럼명 [, N[, 기본값]]) [null_treament] over_clause
    --현재행 기준으로 N행 이전의 칼럼값을 리턴, 없으면 기본값 리턴(생각하면 NULL)
 */
 ...
 FROM EMP;
  -- Q1-1) 급여 높은순으로 정렬하고 각 사원의 {2번쨰 이전 사원 급여} = LAG(SAL,2)를 리턴
  -- 없으면 0 리턴
    SELECT ENAME, SAL, LAG(SAL , 2, 0)  OVER (ORDER BY SAL DESC)  AS res
 FROM EMP;
 
   -- Q1-2) 부서별로 사원 급여 정렬하고 각 사원의 이전 사원 급여 = LAG(SAL,2)를 리턴
  -- 없으면 0 리턴
    
    SELECT DEPTNO, ENAME, SAL,
    LAG(SAL , 1, 0)  OVER (PARTITION BY DEPTNO
							ORDER BY SAL DESC)  AS 이전사원급여
	FROM EMP;
...
FROM EMP;
 ...
 --   FIRST_VALUE(SAL) OVER (PARTITION BY DEPTNO ORDER BY SAL DESC)-> 정렬된 결과에서 첫번째 값 리턴,
 --   LAST_VALUE(SAL) OVER (PARTITION BY DEPTNO ORDER BY SAL DESC 
 --       ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) -> 처음부터 끝짜기 마지막 행 급여값 리턴
 -- 	결론 LAST_VALUE() + ROWS 를 함 꼐 사용한다. 그렇지 않으면 "현재행" 을 리턴함.
...
-- Q14)  사원테이블에서 부서 번호와 각 부서 (DEPTNO) 직원의 평균 재직 기간(년)을 출력 하자.  
     ...
     -- AVG, TIMESTAMPDIFF(unit,datetime1,datetime2): INTEGER , COUNT 사용 
 ...
 -- VAR_SAMP(expr) [over_cluase] - N-1로나눈다
 -- 모집단 분산 VAR_POP(expr) [over_clause} N으로나눈다
   ...
   -- CASE2. 윈도우 함수를 화룡해서 표본분산 출력
   
	SELECT DEPTNO ,   VAR_SAMP(SAL) OVER (PARTITION BY DEPTNO) AS "RES"
    FROM EMP;
    
    SELECT DEPTNO, VAR_POP(SAL) OVER (PARTITION BY DEPTNO) AS "RES"
    FROM EMP;
