...
CREATE DEFINER=`abc`@`localhost` FUNCTION `FUN02`(E_SAL DECIMAL(15,2)) 
...
-- 선언
DELIMITER $$
CREATE DEFINER=`abc`@`localhost` FUNCTION `FUN03`(emp_name VARCHAR(50), hire_date DATE) RETURNS varchar(100) CHARSET utf8mb4
    DETERMINISTIC
BEGIN
...
-- 선언  
 DELIMITER $$
CREATE DEFINER=`abc`@`localhost` FUNCTION `FUN04`(D_DEPTNO INT)
