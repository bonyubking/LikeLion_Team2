...
/* check option : with check opiton 뷰 생성시 추가 옵션, cascaded를 지정하게 되면
 뷰와 관련된 모든 중첩된 뷰의 조건까지 체크하도록한다.!!
 
 IS_UPDATABLE : 수정 가능 유무 리턴. YES는 해당 뷰를 통해 원본 수정할수있음
 
 *** CASCADED , YES 이면 조건에 만족하는 INSERT와 UPDATE 가능
...
-- JOIN_EMP02 생성시 WITH CASCACDED CHECK OPTION,
-- WHERE DEPTNO > 10 을 만족해야 INSERT UPDATE 간으함
...
FROM JOIN_EMP02;
 ...
 -- Q8. https://dev.mysql.com/doc/refman/8.4/en/view-updatability.html
 /* 
 INSERT : 조인 뷰 X -> 원본 테이블을 업데이트
 UPDATE: 조인 뷰 O -> 업데이트 대상 컬럼이 업데이트 가능 테이블에서 올경우
 DELETE : 조인 뷰 X -> 병합뷰가능하지만 조인뷰 불가능
 */
...
-- 8-2 INSERT 해보자 --> 원본으로 돌아오는 데이터의 합을 VIEW 컬럼에서 계산한 결과확인
SELECT * FROM T1;
INSERT INTO T1 VALUES(10);
SELECT * FROM VMAT;

-- 8-3 조인 테이블을 이용한 뷰 생성 확인
...
-- UPDATE 확인 -> JOIN 뷰에서 일부만 업데이트 되도 OK
UPDATE vjoin SET c=c+1; -- T2테이블의 C 컬럼 VUP 컬럼으로 업데이트가능
UPDATE vjoin SET x=x+1; -- VMAT의 SUM(X) 집계 포함 업데이트 불가능


-- 다중테이블 UPDATE 구문 -> vup로 업데이트 대상으로 조인
UPDATE vup JOIN (SELECT SUM(X) AS s FROM t1) AS dt ON...
SET c= c+1;

-- 
UPDATE vup JOIN (SELECT SUM(X) AS s FROM t1) AS dt ON...
SET s= s+1; -- 파생테이블 DT는 집계 수정 불가
