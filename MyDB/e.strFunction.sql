...
SELECT EMPNO, CONCAT(UPPER(SUBSTR(ENAME, INSTR(ENAME, SUBSTR(ENAME,1,1)), 1)),
						LOWER(SUBSTR(ENAME, 2))) AS 껼과
FROM EMP;

-- Q3-2 사원의 이름중 INSTR 사용하서 A 글자 찾아서 소문자 출력
...
-- ex): 'SMITH', 5, 'ALLEN', 5, ...
SELECT CONCAT(ENAME, "의 길이: ", LENGTH(ENAME)) AS 길이
FROM EMP;


-- Q7. 사원번호, 사원의 이름, 연봉의 별칭으로 하나의 문자열로 출력하자. CONCAT
-- ex): '7369SMITH800', '7499ALLEN1600', ...
SELECT CONCAT(EMPNO, " " ,ENAME, " ", SAL) AS 사원정보
FROM EMP;
...
SELECT TRIM(TRAILING ' ' FROM 'barxx1z        ') AS RES;
...
-- ex): 2, 0, 3, ...
SELECT ENAME, LOCATE('A',ENAME,1) AS 위치
FROM EMP;

-- Q22. 사원의 이름에서 세 번째 글자부터 끝까지 추출하여 출력하자. SUBSTRING
-- ex): 'ITH', 'LEN', 'RD', ...
SELECT ENAME, SUBSTRING(ENAME, 3) 
FROM EMP;
...
/* 키값 조심
 ...
 테이블에 새로운 행을 삽입
 기존 행을 삭제하게 새로운 행을 삽입 함 -> INSERT와 방식 다름
 
 CASE 1 : 데이터 중복
 새로운 행을 테이블에 삽입 -> PRIMARY KEY or UNIQUE index값이 중복된 값있다 -> 
 기존 중복 삭제 -< 새로운 행 삽입 
 
 CASE 2 : 데이터 중복 되지 않을경우는 일반 INSERT INTO 문이 추가됨.*/

-- Q24. 사원의 이름에서 모든 'L' 문자를 제거하여 출력하자. REPLACE
-- ex): 'SMITH', 'AEN', 'WARD', ...

SELECT ENAME, REPLACE(ENAME, 'L' , '')
FROM EMP;
...
SELECT LENGTH('A'),LENGTH('TEXT'), LENGTH('가'),LENGTH('한글이란다') ;
...
SHOW VARIABLES  LIKE  'character_set_filesystem'; #파일 크기 제한   
 -- 경로는 NULL, 파일 크기는 'max_allowed_packet', '67108864' 


SELECT LOAD_FILE('C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\APPLE.JPG');
...
DESC MY_FILE;
INSERT INTO MY_FILE VALUES(100,'1');
...
SELECT my_image ID, FILE_CONTENT  
FROM MY_FILE;
...
INSERT INTO MY_IMAGE(IMG_CONTENT)  
  VALUES(LOAD_FILE('C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\A.TXT')),
        (LOAD_FILE('C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\APPLE.JPG'));  
...
-- 데이터베이스 스키마 생성지 문자셋 , 정렬순을 지정해보자.
CREATE DATABASE my_emp_bin
CHARACTER SET utf8mb4
COLLATE utf8mb4_bin;

-- 테이블 생성시 컬럼에 문자셋, 정렬 순을 지정해보자
