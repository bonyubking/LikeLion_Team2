...
-- 8. 2023년 6월 30일에 대출된 도서들 중 누적 대출 횟수가 가장 적은 책 제목을조회합니다. 

SELECT B.TITLE
FROM BOOK B
JOIN RENT_MASTER RM ON B.TITLE= RM.TITLE
WHERE BK_IDX IN (
	SELECT BK_IDX
	FROM RENT_BOOK
	WHERE REG_DATE = '2023-06-28')
ORDER BY RM.RENT_BOOK_CNT ASC
LIMIT 1;

-- 9. 2023년 6월에 대출되고 누적 대출 횟수가 평균보다 작은 도서의 제목과 대출 횟수를 조회

SELECT B.TITLE, B.RENT_CNT
FROM BOOK B
WHERE BK_IDX IN (
	SELECT BK_IDX
	FROM RENT_BOOK
...
-- 17. 각 사용자별로 작성한 게시글 수와 함께 모든 사용자 아이디를 게시글 수를 내림차순으로 
...
SELECT SUM(book_Amt) AS 카테고리수필권수
FROM BOOK
WHERE CATEGORY IN (
	SELECT CODE
    FROM CODE
...
-- 20. 모든 사용자 중에서 가장 먼저 탈퇴한 사용자가 작성한 게시물 중 가장 조회수가 높은 게시
-- 글의 작성자, 게시글, 조회수를 조회합니다. 
