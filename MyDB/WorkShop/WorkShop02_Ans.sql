-- Q1) 책 가격이 높은 순서로 순위를 매겨 
-- 책 제목, 가격과 함께 출력하시오. (RANK 함수 활용)
USE mywork;

SELECT bk_idx, title, book_amt, RANK() OVER (ORDER BY book_amt DESC) 
AS amt_rank 
FROM book;

-- Q2) 카테고리별로 대여 횟수 순위를 매겨 책 제목, 대여 횟수와 함께 출력하시오. 

SELECT bk_idx, category, title, rent_cnt, RANK() OVER (PARTITION BY CATEGORY ORDER BY rent_cnt DESC) AS rent_rank 
FROM book;

-- Q3) 등록일 기준으로 가장 오래된 도서와 가장 최근 도서 제목을 각각 출력하시오. 
-- (FIRST_VALUE, LAST_VALUE 함수 활용)

SELECT bk_idx, title, reg_date, FIRST_VALUE(title) OVER (ORDER BY reg_date ASC) AS oldest_book,
    LAST_VALUE(title) OVER (ORDER BY reg_date ASC) AS newest_book
FROM book
ORDER BY bk_idx ASC;
...
SELECT bk_idx, category, title, book_amt, AVG(book_amt) OVER (PARTITION BY CATEGORY) AS avg_amt,
book_amt - AVG(book_amt) OVER (PARTITION BY CATEGORY) AS diff_avg
FROM book;

-- Q5) 카테고리별 총 대여 횟수를 계산하여 책 제목과 함께 출력하시오. (SUM 함수 활용)

SELECT BK_idx, category, title, rent_cnt, SUM(rent_cnt) OVER (PARTITION BY CATEGORY) AS total_rent
FROM book;
...
-- Q9) 제목 길이가 10자 이상인 도서의 제목과 글자 수를 출력하시오. (CHAR_LENGTH 함수)

SELECT title, CHAR_LENGTH(title) as len
FROM book
WHERE CHAR_LENGTH(title) >= 10;

-- Q10) 도서 가격에 부가세(10%)를 추가한 금액을 계산하고, ₩ 기호가 붙은 통화 형식으로 변환
-- 하여 책 제목과 함께 출력하시오.

SELECT TITLE, BOOK_AMT, CONCAT('W', book_amt*1.1) AS price_with_tax
FROM book;

-- Q11) 등록일로부터 현재까지 경과한 날짜를 계산하여 책 제목과 함께 출력하시오. (DATEDIFF 
-- 함수 활용)

 SELECT TITLE, REG_DATE, DATEDIFF(CURRENT_DATE, reg_date) as days_passed
 FROM book;
 
-- Q12) 대여 횟수의 최대값과 각 도서의 대여 횟수 차이를 계산하여 책 제목과 함께 출력하시오. 
-- (MAX 함수 활용)

SELECT TITLE, RENT_CNT, MAX(RENT_CNT) OVER () - RENT_CNT AS diff_max_rent
FROM book; 
...
SELECT TITLE
FROM book
WHERE LOCATE('SQL', TITLE) > 0; -- LOCATE함수는 위치반환

--  Q15) 도서 가격의 10%를 할인한 금액을 계산하여 책 제목과 함께 출력하시오. (ROUND 함수 
--  활용)

SELECT TITLE, BOOK_AMT, ROUND(BOOK_AMT*0.9) AS discounted_price
FROM book;

-- Q16) 도서 가격을 천 단위로 절삭하여 책 제목과 함께 출력하시오. (FLOOR 함수 활용) 

SELECT TITLE, BOOK_AMT, FLOOR(BOOK_AMT/1000) * 1000 AS discounted_price
FROM book;
...
-- Q20) 전체 도서의 종류 개수와 각 도서의 제목을 함께 출력하시오. (COUNT() OVER() 활용) 
...
SELECT TITLE, RENT_CNT, LEAD(RENT_CNT) OVER (ORDER BY rent_cnt asc) as Next_rent_cnt
FROM  book;

-- Q26) 도서 가격에 따라 가격 등급을 '고가', '중가', '저가'로 구분하여 책 제목과 함께 출력하시
-- 오. (CASE WHEN 활용)
SELECT TITLE, BOOK_AMT, CASE WHEN
	BOOK_AMT >= 10 THEN '고가'
    WHEN BOOK_AMT >= 5 THEN '중가'
    ELSE '저가'
    END AS price_level
 FROM book;   
...
-- Q28) 등록일 기준으로 누적 도서 수를 계산하여 책 제목과 함께 출력하시오. (COUNT + 윈도
-- 우 함수 활용) 

SELECT TITLE, REG_DATE, COUNT(*) OVER (ORDER BY REG_DATE ASC) as cumulative_count
FROm book; 
...
-- Q30) 대여 횟수 순위를 매겨 동일 값이면 같은 순위로 책 제목과 함께 출력하시오. (RANK 함
-- 수 활용)
